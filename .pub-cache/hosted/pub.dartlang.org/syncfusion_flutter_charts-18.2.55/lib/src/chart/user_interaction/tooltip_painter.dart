...
class _TooltipPainter extends CustomPainter {
    ...
    if (chart._chartAxis._axisClipRect.contains(position)) {
      ...
      if (isContains) {
        ...
        if (tooltip.shared) {
          ...
          for (int j = 0;
              j < chart._chartSeries.visibleSeriesRenderers.length;
              j++) {
            final CartesianSeriesRenderer seriesRenderer =
                chart._chartSeries.visibleSeriesRenderers[j];
            if (seriesRenderer._visible &&
  ...
  String _calculateCartesianTooltipText(
    ...
    if (tooltip.format != null) {
      resultantString = (seriesRenderer._seriesType.contains('range') || seriesRenderer._seriesType == 'hilo') && !isTrendLine
          ? (tooltip.format.replaceAll('point.x', values[0]).replaceAll('point.high', _getLabelValue(point.high, axis, digits)).replaceAll('point.low', _getLabelValue(point.low, axis, digits)).replaceAll(
  ...
  void _renderCircularChartTooltip(Offset position) {
    ...
    bool isContains = false;
    final _Region pointRegion = _getPointRegion(
        chart, position, chart._chartSeries.visibleSeriesRenderers[0]);
    if (pointRegion != null &&
  ...
  void _renderTriangularChartTooltip(Offset position) {
    ...
    if (chart.tooltipBehavior.enable) {
      ...
      final dynamic chartPoint = chart._chartSeries
  ...
  void _renderTooltip(Canvas canvas) {
    ...
    if (x != null &&
      ...
      tooltipArgs.seriesIndex = _chart is SfCartesianChart
          ...
          : currentSeries;
      tooltipArgs.dataPoints = _chart._chartSeries
          .visibleSeriesRenderers[tooltipArgs.seriesIndex]._dataPoints;
