...
class _TtfReader {
  ...
  Map<int, _TtfGlyphInfo>? _unicodeUCS4GlyphCollection;
  int? _lowestPosition;
  ...
  List<String>? _internalUsedChars;
  ...
  Map<int, _TtfGlyphInfo>? get _unicodeUcs4Glyph {
    _unicodeUCS4GlyphCollection ??= <int, _TtfGlyphInfo>{};
    return _unicodeUCS4GlyphCollection;
  }

  List<String> get _usedChars {
    _internalUsedChars ??= <String>[];
    return _internalUsedChars!;
  }
  ...
  void _readUCS4CmapTable(
    ...
    final _TtfTableInfo tableInfo = _getTable('cmap')!;
    _offset = tableInfo.offset! + subTable.offset + 12;
    final int count = _readInt32(_offset!);
    for (int i = 0; i < count; ++i) {
      final int start = _readInt32(_offset!);
      final int end = _readInt32(_offset!);
      int glyphID = _readInt32(_offset!);
      ...
      if (!_metrics!.isSymbol && _microsoft != null) {
        if (_microsoft!.containsKey(code)) {
          ...
          if (_unicodeUCS4GlyphCollection != null && _isSurrogate(code)) {
            _TtfGlyphInfo newGlyph = _TtfGlyphInfo();
            ...
            if (_isLowSurrogate(code)) {
              if (_isSurrogatePair(_surrogateHigh, code)) {
                ...
                if (_unicodeUCS4GlyphCollection!.containsKey(code)) {
                  ...
                  if (isSetSymbol!) {
                    _usedChars.add(_surrogateHighChar + char);
                  }
                }
              }
            }
  ...
  void _addReverseGlyph(_TtfGlyphInfo glyph, _TtfCmapEncoding encoding) {
  ...
  Map<int, int> _getGlyphChars(List<String> chars) {
  ...
  List<int>? _readFontProgram(List<String> chars) {
  ...
  String _convertString(String text) {
    ...
    for (int k = 0; k < text.length; k++) {
      ...
      final _TtfGlyphInfo glyphInfo = _getGlyph(char: char, isSetSymbol: true)!;
      if (!glyphInfo.empty) {
